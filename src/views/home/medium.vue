<template>
  <div class="medium" @click="toBottom">
    <div class="medium-title">
      Pick Your Electron!
    </div>
    <svg class="medium-title-icon" viewBox="0 0 1035 1024" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  methods: {
    runToBottom() {
      ...
      currentPosition += 10;
      if (currentPosition < this.scrollHeight) {
    ...
    toBottom() {
      this.timer = setInterval(this.runToBottom,1)
    }
  },
  mounted() {
    this.scrollHeight = document.body.clientHeight - document.documentElement.clientHeight;
    // document.body.clientHeight 是页面总高度。
    // document.documentElement.clientHeight 是窗口高度。
  }
}
</script>
